...
def validate_gguf_pair(model_path: str | Path, mmproj_path: str | Path, *, max_bytes: int = MAX_ALLOWED_MODEL_BYTES) -> GGUFModelInfo:
    model = Path(model_path); mmproj = Path(mmproj_path)
    for path in (model, mmproj):
        if not path.is_file(): raise LlamaServerVLMError(f"GGUF file does not exist: {path}")
        if path.suffix.lower() != ".gguf": raise LlamaServerVLMError(f"Expected a .gguf file: {path}")
...
def _get_json(url: str, *, timeout: float) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict): raise LlamaServerVLMError(f"Unexpected JSON response from {url}: {payload!r}")
    return payload
...
class LlamaServerVisionObserver:
    def __init__(self, model_path: str | Path, mmproj_path: str | Path, *, base_url: str = "http://127.0.0.1:8080/v1", model_name: str = "SmolVLM2-2.2B-Instruct", timeout: float = 120.0, max_new_tokens: int = 180, max_model_bytes: int = RECOMMENDED_MAX_MODEL_BYTES) -> None:
        ...
        self.base_url = base_url.rstrip("/"); self.model_name = model_name; self.timeout = timeout; self.max_new_tokens = max_new_tokens
    ...
    def describe(self, image_path: str | Path, *, visual_record: str = "", crop_reason: str = "whole_image") -> dict[str, Any]:
        self.check_server()
        prompt = ("Analyze this scientific or technical diagram region. Give a concise semantic hypothesis about what it shows, "
        ...
        body = {"model": self.model_name, "messages": [{"role": "user", "content": [
            {"type": "text", "text": prompt}, {"type": "image_url", "image_url": {"url": _data_uri(image_path)}}
        ]}], "temperature": 0.1, "max_tokens": self.max_new_tokens}
        request = urllib.request.Request(f"{self.base_url}/chat/completions", data=json.dumps(body).encode("utf-8"), headers={"Content-Type": "application/json", "Accept": "application/json"}, method="POST")
        try:
        ...
        except (KeyError, IndexError, TypeError) as exc: raise LlamaServerVLMError(f"Unexpected llama.cpp response schema: {payload!r}") from exc
        if not isinstance(text, str) or not text.strip(): raise LlamaServerVLMError("llama.cpp vision server returned an empty semantic response")
...
def enrich_scene_with_llama_server_vlm(scene: Any, image_path: str | Path, model_path: str | Path, mmproj_path: str | Path, *, base_url: str = "http://127.0.0.1:8080/v1", model_name: str = "SmolVLM2-2.2B-Instruct", max_crops: int = 8, max_model_bytes: int = RECOMMENDED_MAX_MODEL_BYTES) -> Any:
    ...
    observer.check_server(); crops = select_semantic_crops(scene, image_path, max_crops=max_crops)
    if not crops:
        scene.warnings.append("LLAMA_SERVER_VLM skipped: no high-value semantic crops were selected."); return scene
    temp_dir = Path(image_path).parent / ".semantic_crops_llama"; hypotheses: list[dict[str, Any]] = []
    try:
        for crop in crops:
        ...
        for path in temp_dir.glob("*.png"): path.unlink(missing_ok=True)
        try: temp_dir.rmdir()
        except OSError: pass
