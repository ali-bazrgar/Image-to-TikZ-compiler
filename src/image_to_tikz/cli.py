...
def main() -> int:
    parser = argparse.ArgumentParser(description="Convert an image into a visual representation for any downstream LLM.")
    ...
    parser.add_argument("--prompt", help="Write the strict Golden Prompt for downstream TikZ reconstruction")
    ...
    if args.context: Path(args.context).write_text(context, encoding="utf-8")
    if args.prompt: Path(args.prompt).write_text(build_golden_prompt(scene), encoding="utf-8")
    if args.debug_image:
